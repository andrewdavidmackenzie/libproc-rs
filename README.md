...
In order to have tests pass when not run as `root`, some tests check if they are `root`
at run-time (using our own `am_root()` function) and avoid failing if *not* run as `root`.
